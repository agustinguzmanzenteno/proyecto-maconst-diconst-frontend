...
import { DollarSign, ShoppingCart, Edit, Trash2, Search, Plus, ChevronDown, Check } from 'lucide-react';
...
const Ventas = () => {
  ...
  const fetchTotales = async () => {
    try {
      const res = await apiFetch('/api/ventas/totales');
      setTotalVentas(res.totalVentas);
  ...
  useEffect(() => {
    const fetchVentas = async () => {
      try {
        ...
        const res = await apiFetch(`/api/ventas?${params.toString()}`);
        setItems(res.data);
  ...
  const handleSubmitModal = async (payload) => {
    try {
      if (modalMode === 'create') {
        await apiFetch('/api/ventas', { method: 'POST', body: payload });
        toast.success('Venta creada con éxito');
      } else {
        await apiFetch(`/api/ventas/${editing.id}`, { method: 'PUT', body: payload });
  ...
  return (
    <div className={`min-h-screen p-6 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <div className="mb-6">
        <h1 className={`text-2xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Ventas</h1>
        <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'} mt-2`}>Listado de ventas</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className={`p-6 rounded-xl ${isDark ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'} text-sm font-medium`}>Ingresos Totales</p>
        ...
        <div className={`p-6 rounded-xl ${isDark ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'} text-sm font-medium`}>Ventas Totales</p>
              <p className={`${isDark ? 'text-white' : 'text-gray-900'} text-2xl font-bold mt-2`}>
                {totalVentas.toLocaleString('en-US')}
              </p>
            </div>
      ...
      <div className={`rounded-xl ${isDark ? 'bg-gray-800' : 'bg-white'} shadow-lg overflow-hidden`}>
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
          <h2 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Historial de Ventas</h2>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className={`divide-y ${isDark ? 'divide-gray-700' : 'divide-gray-200'}`}>
                  {items.map((v) => (
                    <tr key={v.id} className={`hover:${isDark ? 'bg-gray-700' : 'bg-gray-50'} transition-colors`}>
                      <td className={`px-6 py-3 text-sm ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{v.id}</td>
                      <td className={`px-6 py-3 text-sm ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{v.codigo}</td>
                      <td className={`px-6 py-3 text-sm ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{v.mes}</td>
                      <td className={`px-6 py-3 text-sm ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{v.anio}</td>
                      <td className={`px-6 py-3 text-sm ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{v.id_producto}</td>
                      <td className={`px-6 py-3 text-sm ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{v.cantidad}</td>
                      <td className={`px-6 py-3 text-sm ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{fmtNum(v.precio_unitario)} Bs.</td>
                      <td className={`px-6 py-3 text-sm ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{fmtNum(v.precio_total)} Bs.</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={() => handleDeleteConfirmation(v.id)}
                  ...
                  {items.length === 0 && (
                    <tr>
                      <td colSpan="9" className="px-6 py-6 text-center text-gray-600">No se encontraron ventas</td>
                    </tr>
                  )}
                </tbody>
              </table>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => deleteVenta(ventaToDelete)}
        isDark={isDark}
      />
    </div>
  );
